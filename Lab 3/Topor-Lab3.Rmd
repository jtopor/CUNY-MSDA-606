...
The distributions show that heights of men and women both appear to be nearly normally distributed as indicated by there nearly bell-shaped, symmetric shapes. However, the distributions also show that the heights of males are spread over a wider range (155 -> 200 = approx. 45cm) than are the heights of females (145 -> 185 = approx 40cm). Also, the mean height of males (approx. 178cm) appears to be 10 to 15 cm greater than that of females (approx 165cm).
...
Yes, the data do appear to follow a nearly normal distribution.
...
All of the points do not fall on the line. The plot varies from that of the actual data in that the actual data appear to be somewhat "stepped", likely due to the binning of samples into specific quantiles.
...
Yes, the actual data for female heights does appear to approximate a normal distribution.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.

We'll start by creating a normal probability plot for female weights:  
```{r}
qqnorm(fdims$wgt)
qqline(fdims$wgt)
```

From this plot it appears as though the female weight data do not come from a normal distribution. For example, weights above the "+1" quantile skew quite drastically away from the line. This is typically an indication that a distribution is right-skewed. We can examine whether this is in fact the case by plotting a histogram of female weights:  
```{r}
hist(fdims$wgt)
```

The histogram shows clear evidence of right skew in the distribution of female weights. 

Finally, we can compare our normal probability plot for female weights against several simulated normal probability plots based on the mean and standard deviation of the female weight data:  

```{r}
qqnormsim(fdims$wgt)
```

The simulated plots hew much more closely to the normal line than does the actual female weight data. As such, it appears that our female weight data does not come from a normal distribution.
...
The two questions are as follows:  
  ...
  + What is the probability that a female's height is less than 66 inches (i.e., 5 feet, 6 inches) ?  

  + What is the probability that a female's weight is greater than 150 pounds?

Since both of these questions makes use of metrics that do not conform to the data (i.e., inches and pounds vs. centimeters and kilograms in the actual data), we'll need to be sure to convert the data from our questions to the metrics provided in the actual data.

We'll start with the height question:
```{r}
# Answer using theoretical method:
# Convert inches to centimeters: 2.54 cm per inch
qfHgt <- 66 * 2.54

# get the theoretical probability
theo_prob <- pnorm(q = qfHgt, mean = fhgtmean, sd = fhgtsd)
theo_prob
```
Using R's __pnorm__ function we get a theoretical probability of 0.66 for the likelihood that a female is less than 5 foot 6 inches tall.

```{r}
# Answer using empirical method:
# get the empirical probability
emp_prob <- sum(fdims$hgt < qfHgt) / length(fdims$hgt)
emp_prob
```
Empirically, we find that the probability of a female being less than 5 foot 6 inches tall is 0.68.

Subtracting the theoretical probability from our empirical probability we find a difference between the two of 0.017:
```{r}
abs(emp_prob - theo_prob)
```
  
  
Turning to the weight question:  

```{r}
# convert pounds to kilos: .453 kilos per pound
qfWgt <- 150 * .453

# find mean + sd for female weights
fwgtmean <- mean(fdims$wgt)
fwgtsd   <- sd(fdims$wgt)

# get the theoretical probability of > 150 pounds
theo_wprob <- 1 - pnorm(q = qfWgt, mean = fwgtmean, sd = fwgtsd)
theo_wprob
```

Using R's __pnorm__ function we get a theoretical probability of 0.22 for the likelihood that a female's weight is greater than 150 pounds.  

```{r}
# Answer using empirical method:
# get the empirical probability
emp_wprob <- sum(fdims$wgt > qfWgt) / length(fdims$wgt)
emp_wprob
```
Empirically, we find that the probability of a female's weight being more than 150 pounds as 0.19.

We then find a difference between the two of 0.03:
```{r}
abs(theo_wprob - emp_wprob)
```


As we can see, the weight data (.03) varies more from a purely normal distribution than does the height data (.017), so the height data has closer agreement between the theoretical and empirical methods.
...
The plots for each of these were generated using the following R code:
    
```{r, eval=FALSE}
# a. female biiliac (pelvic) diameter bii.di plot match
qqnorm(fdims$bii.di)
qqline(fdims$bii.di) 
# matches plot B    

# b. female elblow diameter eld.di plot match
qqnorm(fdims$elb.di)
qqline(fdims$elb.di) 
# matches plot C

# c. age plot match (gender neutral)
qqnorm(bdims$age)
qqline(bdims$age) 
# matches plot D

# d. female chest depth che.de plot match
qqnorm(fdims$che.de)
qqline(fdims$che.de) 
# matches plot A
```
...
The step pattern is a result of the "binning" of samples into specific quantiles.  
...
```{r}
#3. Make a normal probability plot for female knee diameter (kne.di).
qqnorm(fdims$kne.di)
qqline(fdims$kne.di) 
```
  
The normal probability plot appears to indicate that knee diameter is right skewed. A histogram plot confirms this:
