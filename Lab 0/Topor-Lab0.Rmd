...
```{r load-abrbuthnot-data, eval=TRUE, echo=FALSE}
# setwd("C:/Users/Hammer/Documents/Lab0")
...
__We can extract just the counts of girls baptized by referencing solely the $girls variable contained within the arbuthnot dataframe:__
```{r}
arbuthnot$girls
```
...
__Annual baptisms of girls appear to have plummeted shortly after the year 1640 and did not full rebound until about 20 years later. After 1660 the number of baptisms of girls climbed steadily until plateauing in the mid 1680's. A sharp one-year drop was seen for in the early 1700's after which the number of baptisms of girls returned to its plateau.__
...
To find the years included in the data set we can simply list the values of the present$year variable:
```{r}
present$year
```
The years 1940 - 2002 are included in this data set

We can find the dimensions of the data set as follows:
```{r}
dim(present)
```
...
They are not on a similar scale. For example, comparing the average number of boys for each timeframe shows that, on average, the number of boys baptized during the period of 1940 - 2002 is 319.2092 times larger than the number of boys baptized during the arbuthnot period:
...
The plot does appear to show that more boys are born each year in the U.S. than girls for the period 1940 - 2002. For each and every year plotted we see a value greater than one for the ratio 'boys/girls'.
...
We can use the __which.max__ function to help us find the year that had the most total number of births in the U.S.:
```{r}
# what year had most births?
present[which.max(present$boys + present$girls), ]
```

It appears that 1961 had the most total births in the U.S.  
