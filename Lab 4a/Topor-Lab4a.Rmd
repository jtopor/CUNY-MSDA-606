...
setwd("C:/Users/Hammer/Documents/Lab4a")
...
The distribution __N(mu = 1500, sd = 505.5)__ with a median of 1442 is right skewed as evidenced by the fact that the value of the mean exceeds the value of the median by a meaningful amount. Further evidence of right skew is obvious in the appearance of the histogram.
...
We can make use of R's __summary__, __hist__, and __sd__ functions to help us describe the sample distribution.
```{r}
summary(samp1)
hist(samp1)
sd(samp1)
```
The distribution of the sample is similar, though not identical, to that of the population. The sample distribution is right skewed as evidenced by the median being meaningfully less than the mean. The shape of the histogram provides additional evidence of right skew.
...
# How does mean of samp2 compare to mean of samp1?
mean(samp2)
```

The mean of samp2 is not identical to that of samp1. 

If we took larger samples, the sample means would theoretically be more likely provide a more accurate measure of the population mean. However, there is no guarantee that the means of those samples would actually be more representative of the population mean. We can see the results of representative samplings of size 100 and 1000 here:
```{r}
...
Sample_means50 can be summarized as: N(mu = 1500, sd = 71.62) with a median = 1498. The median of the sample means is very close to the mean of the sample means, which appears to indicate that this distribution of sample means is nearly normal.

We should not expect the distribution to change much if we collect 50,000 sample means. Since there are only 2930 elements in the population it is unlikely that collecting an additional 45,000 sample means above and beyond the 5,000 we've already collected will alter the distribution.
...
There are 100 elements in the object. Each element represents the mean of a sample of size 50 taken from the 'area' population.
...
The center of the distribution doesn't appear to change as the sample size increases for this set of data. The spread of the distribution of sample means appears to decrease when the sample size is larger as evidenced by the narrower range of values displayed in the histogram.
...
The best point estimate of the population mean is shown above.
...
```{r}
# initialize a vector of 5000 zeros
sample_means50 <- rep(NA, 5000)

#  take 5000 samples of size 50 from the population, calculate the mean of 
#  each sample, and store each result in a vector called sample_means50
for(i in 1:5000){
  samp <- sample(price, 50)
  sample_means50[i] <- mean(samp)
}
...
The plot of the sampling distribution shows it to be a normal-shaped distribution. Based on the histogram, I would guess the mean home price to be approximately $180,000
...
Therefore, the mean of the sampling distribution does appear to approximate the mean of the actual population, which is $180,796.
...
The plot of the sampling distribution shows it to be a normal-shaped distribution. 

We can compare the sampling distributions by plotting their histograms together:
```{r}
par(mfrow = c(2, 1))

# Compare sample_means50 to sample_means150
...
The spread of the distribution for a sample size of 150 appears to be narrower than that of the distribution for a sample size of 50. Based on the sampling distribution I would guess the mean sale price of homes in Ames to be approximately $180,000.
...
As shown in the histograms plots above, the distribution from 3 has the smaller spread. We'd prefer smaller spreads when estimating values since smaller spreads are indication of better precision.
