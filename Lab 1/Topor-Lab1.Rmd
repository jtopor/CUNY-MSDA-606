...
```{r load-data}
# setwd("C:/Users/Hammer/Documents/Lab1")
...
We can use R's __str__ function to find some of the answers to these questions:
```{r}
str(cdc)
```
There are 20,000 cases and 9 variables. 

The data type for each variable is as follows:

- genhlth:  Categorical, ordinal
- exerany:  Categorical
- hlthplan: Categorical
- smoke100: Categorical
- height:   numeric, discrete (since no fractional heights are present)
- Weight:   numeric, discrete (no fractional weights are present)
- wtdesire: numeric, discrete (no fractional desired weights are present)
- age:      numeric, discrete (no fractional ages are present)
- gender:   Categorical
...
Summary stastistics and interquartile ranges for 'height' and 'age' are:
```{r}
# gen sum stats + calc IQR for height
summary(cdc$height)
IQR(cdc$height)

# gen sum stats + calc IQR for age
summary(cdc$age)
IQR(cdc$age)
```

The relative frequency distributions for 'gender' and 'exerany' are:
```{r}
# relative frequency of categorical variable gender
table(cdc$gender)/20000

# relative frequency of categorical variable exerany
table(cdc$exerany)/20000
```

The number of males in the sample is 9569:
```{r}
# how many males in sample?
table(cdc$gender == 'm')
```

The number of respondents that said they were in excellent health is 4657:
```{r}
# how many respondents said they were in excellent health?
...
The mosaic plot reveals that a larger proportion of males have smoked at least 100 cigarettes than have females. 
...
```{r}
# get data for all respondents < 23 yrs old and smoke
under23_and_smoke <- subset(cdc, age < 23 & smoke100 == TRUE)
...
The boxplot shows that people who identify themselves as being in relatively better states of general health tend to have lower body mass indices. As we can see in the plot, those in excellent health had the lowest BMI's and BMI values increase as general health descriptions decline from 'excellent' toward 'poor'.

We can also examine the relationship between exercise and BMI. From an intuitive perspective, regular exercise is usually associated with people having less body fat, and therefore a lower BMI:
```{r}
boxplot(bmi ~ cdc$exerany)
```

The boxplot of this relationship shows that people who claim to exercise tend to have lower BMI's than do people who do not exercise.   
...
```{r}
# scatter plot of weight vs desired weight
plot(x = cdc$weight, y = cdc$wtdesire)
```

The scatterplot indicates a modest positive relationship between weight and desired weight. In other words, the higher an individual's weight is, the higher their likely desired weight tends to be.
...
```{r}
# create a new var wdiff = desired weight - weight = how much weight
# a respondent wants to lose
...
wdiff is a numerical discrete variable. If an observation 'wdiff' is 0, it means the person's weight and desired weight are identical. As such, that person is content with their present weight. If 'wdiff' is positive it indicates that the person wishes to gain weight, whereas if 'wdiff' is negative the person would like to lose weight.
...
We can examine the distribution of 'wdiff' using R's __summary__ function and by plotting a histogram:    
```{r}
summary(wdiff)
hist(wdiff)
```

The output of R's __summary__ function tells us that the mean of the distribution (-14.59) is less than the median (-10), which is generally an indicator of the distribution being left-skewed. Our histogram confirms this, showing a nearly normal distribution that is skewed left, apparently due to the desire of some respondents to lose more than 150 pounds of weight.

In general, the distribution tells us that people on average would like to lose weight, with the average desired weight loss being 14.59 pounds and the median desired weight loss being 10 pounds.  
...
```{r}
wdiffMen <- subset(cdc, gender == "m")$wtdesire - subset(cdc, gender == "m")$weight
summary(wdiffMen)

wdiffWomen <- subset(cdc, gender == "f")$wtdesire - subset(cdc, gender == "f")$weight
summary(wdiffWomen)

boxplot(wdiffMen, wdiffWomen)
```

From the summaries shown above we see that, on average, men appear to desire less weight loss than do women. Women's mean desired weight loss is 18.15 pounds with a median of 10 pounds while men's mean desired weight loss is 10.71 pounds with a median of 5 pounds.
...
# need to find Z values corresponding to 1 and -1

# Find z = 1
# (x - 169.6) / 40.08 = 1 => x - 169.6 = 40.08 => x = 40.08 + 169.6 = 209.68
40.08 + 169.6

# Find z = -1
# (x - 169.6) / 40.08 = -1 => x - 169.6 = -40.08 => x = -40.08 + 169.6 = 129.52
-40.08 + 169.6

# then calc % of weights within 1 stddev of mean
pnorm(209.68, mean(cdc$weight), sd(cdc$weight)) - pnorm(129.52, mean(cdc$weight), sd(cdc$weight))

```

0.68267 of the weights are within one standard deviation of the mean.
