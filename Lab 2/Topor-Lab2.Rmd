...
```{r load-data}
setwd("C:/Users/Hammer/Documents/Lab2")
...
There is one hit and one miss in a streak of length 1. A streak of length 0 indicates a missed shot that immediately followed a previous missed shot.
...
We can make use of R's native __summary__ function to find the answers to these questions:
```{r}    
summary(kobe_streak)
```

As we can see, the average streak length was 0.7632 'Hits' and the median length was zero. The longest streak was 4 baskets.
...
We can tabulate the number of heads and tails for 100 flips the unfair coin using R's __table__ function:
```{r}
table(sim_unfair_coin)
```
The number of flips that came up heads is indicated above in the column labeled __heads__.
...
We need to add the __prob = c(.045, .055)__ argument to the sample function:
```{r}
# Exercise 4: adjust prob to kobe's shooting perc + run for 133 shots
...
The distribution is right skewed and zero bound on the lefthand side. The typical streak length can be found using R's __summary__ function:

```{r}
summary(sim_streak)
```

The average streak length for the simulated independent shooter is approximately 1 basket. The longest streak for this particular simulation is shown above in the output of R's __summary__ function in the column labeled __Max.__.  
...
We would expect the distribution to be somewhat similar in its degree of skewness with either more or fewer streaks at the extreme right. Since the events are independent there shouldn't be any drastic change to the distribution if the simulation is repeated.
...
The streak lengths for the independent shooter are somewhat varied from Kobe's but not drastically so. In fact, the two distributions are very similar, thus supporting the idea that "hot hands" is a fallacy for Kobe Bryant, i.e., each of Mr. Bryant's shooting events really is an independent event. 

However, we cannot generalize this statement to the broader population of professional basketball players since the only data we were provided here for this analysis is Kobe Bryant's. Other basketball players may or may not have basket shooting histories that reflect Mr. Bryant's apparent lack of a "hot hand".
